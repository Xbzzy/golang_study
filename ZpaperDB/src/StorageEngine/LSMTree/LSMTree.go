...
func (lsm *LSMTree) initLSMTree() (*LSMTree, error) {
	...
	tree.writeAheadLog = make([]*os.File, 4)
...
func (lsm *LSMTree) writeRestoreLog(key []byte, value []byte, preFix string) error {
	...
	_, err1 := lsm.writeAheadLog[lsm.treeNum].Write(data)
	if err1 != nil {
		return err1
	}
	return nil
...
func MakeLSMTree() (*LSMTree, error) {
	...
	go func() {
		lsmTree.mu.Lock()
		defer lsmTree.mu.Unlock()
		for {
			for i := 0; i < len(lsmTree.memoryTree); i++ {
				if lsmTree.memoryTree[i] != nil {
					lsmTree.memoryTree = lsmTree.memoryTree[i:len(lsmTree.memoryTree)]
					lsmTree.treeNum = 0
					lsmTree.writeAheadLog = lsmTree.writeAheadLog[i:len(lsmTree.memoryTree)]
				}
			}
			randTime := 100 + rand.Intn(randTimeSection)
...
func (lsm *LSMTree) Delete(args *DeleteArgs, reply *DeleteReply) error {
	memoryTree := lsm.memoryTree[lsm.treeNum]
	_, result := memoryTree.root.search(args.Key)
	if result == true {
		memoryTree.delete(args.Key)
	} else {
		start, end, segNum := lsm.searchInSparseHash(args.Key)
		for i := 0; i < len(start); i++ {
			if end[i] == -1 {
			...
			if err != nil {
				...
				if bytes.Compare(decodingData[keyIndex].key, args.Key) == 0 {
					...
					_, err4 := lsm.dataFile[segNum[i]].WriteAt(tmpData, start[i])
					if err4 != nil {
						log.Fatal(err4)
					}
					reply.Deleted = true
